...
class Game:
    ...
    def __init__(self, mode):
        ...
        self.highscores = highscore.Highscore() # High score
        self.forceQuit = False # Quit
        self.mode = mode # Game mode (Single/Multi)
        ...
        if self.mode == self.c.MULTI:
            preloader = pygame.image.load(
                self.c.IMAGE_PATH + "loading.png").convert()
        ...
        while not self.exitGame:
            ...
            self.clearBackground() # Clear background
    ...
    def initMultiSelf(self, data):
        ...
        self.user = player.Player(
            "Player " + ary[0], "p_"+ary[0]+"_", ary[2], self.getMultiStartPosition(ary[0]))
    ...
    def resetGame(self):
        self.field = None # Game field
        self.enemies = [] # Enemies
        self.bombs = [] # Bombs
        self.resetTiles = [] # Tiles  
    ...
    def initGame(self):
        if self.mode == self.c.SINGLE:
            self.printText("Level %d-%d" % (self.stage, self.level), (40, 15)) # Print level
            self.field = board.Board(self.stage, self.level) # Draw map
            self.timer = 3*60+1 # Timer
        elif self.mode == self.c.MULTI:
            self.printText("Multiplayer", (40, 15))
            self.field = board.Board(0, 0)
            self.timer = 5*60+1

        self.drawBoard() # Draw map
        self.drawInterface() # Draw interface
        self.updateTimer() # Update timer

        # players do not have to be reinitialized in single player after the first time
        if self.firstRun:
            self.firstRun = False
            self.initPlayers() # Init players
        else:
            self.resetPlayerPosition(self.user, False) # Reset

        # no enemies in multiplayer
        if self.mode == self.c.SINGLE:
            self.initEnemies() # Init enemies
    ...
    def initPlayers(self):
        if self.mode == self.c.SINGLE:
            self.user = player.Player("Player 1", "p_1_", 0, (40, 40)) # Player class
            self.players.append(self.user) # Append to players list
            self.blit(self.user.image, self.user.position) # Draw player
        elif self.mode == self.c.MULTI:
    ...
    def initEnemies(self):
        ...
        for i in range(0, 5):
            while True:
                ...
                # Check enemies can pass (tile.py)
            ...
            self.enemies.append(e) # Append to enemies list
            self.blit(e.image, e.position) # Draw enemies
    ...
    def runGame(self):
        ...
        while self.gameIsActive:
            ...
            if cyclicCounter % 5 == 1:
                self.clearExplosion() # Clear bomb
    ...
    def gameover(self, player):
        if self.mode == self.c.SINGLE:
            ...
            self.highscores.addScore(player.score) # Add high score
